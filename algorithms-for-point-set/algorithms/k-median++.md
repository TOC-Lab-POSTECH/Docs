...
\[ cost(P, C) = \sum\_{p \in P} w(p), d(p, C), \quad d(p, C) = \min\_{c \in C} |p - c|. ]
...
\[ x^{(t+1)} = \frac{\sum\_{p \in S} \frac{w(p), p}{|p - x^{(t)}|\}} {\sum\_{p \in S} \frac{w(p)}{|p - x^{(t)}|\}}. ]
...
where (T) is the number of Lloyd iterations, which is typically small in practice.

***

#### Overall StructureThis function executes the full k-median++ algorithm, including initialization and Lloyd-style refinement, and returns the final set of centers.Here (n = |P|) is the number of points and (|C|) denotes the size of a cluster.
...
* **Clustering on representative sets**
  * Applied to compressed representative sets (R\_j) produced by the range-based preprocessing stage.
